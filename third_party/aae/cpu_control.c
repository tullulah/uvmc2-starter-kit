...
#ifdef AAE_PERF_ON
#include "aae_fast.h"
volatile unsigned int aae_perf_us[16] AAE_SRAM_FAST, aae_perf_n[16] AAE_SRAM_FAST;
#endif
/* NOTHING HERE CAN RUN A 6809, AND THREE PORTS PAY FOR IT EVERY FRAME.
 *
 * `CPU_6809` appears nowhere in this file: exec_cpu, cpu_setcontext, cpu_getcontext,
 * cpu_resetter, process_pending_interrupts and cpu_do_interrupt all have no case for it.
 * A 6809 game runs through cpuintrf.c's `cpu_run` instead (starwars.c calls it), so for
 * starwars/esb/tomcatsw this function spent `cpu_divisions * num_cpus` turns -- 816 a
 * frame -- switching contexts that do not exist and accumulating tick counters that
 * `cpu_getcycles` cannot fill for this CPU type either. MEASURED at 1.4 ms of a 23.6 ms
 * frame on starwars: 6% of it, for nothing.
 *
 * The counters it keeps (vid_tickcount, tickcount, the eterna/hertz ticks) are read only
 * by mhavoc.c and tempest.c, both 6502 games, which still take the loop below. */
static int driver_has_runnable_cpu(void)
{
    int i;
    for (i = 0; i < num_cpus; i++)
        switch (driver[gamenum].cpu_type[i]) {
        case CPU_MZ80: case CPU_6502Z: case CPU_6502: case CPU_68000: return 1;
        default: break;
        }
    return 0;
}
...
void run_cpus_to_cycles()
{
  int  x; 
  int cycles_ran=0;
  unsigned int t_rc_ = AAE_T();

  if (!driver_has_runnable_cpu()) { AAE_ACUM(6, t_rc_); return; }
...
//log_it("CPU(%i) - cycles to Run: %i * %i\n", running_cpu,driver[gamenum].cpu_divisions[0], cyclecount[0]);
  for (x=0; x < driver[gamenum].cpu_divisions[0]; x++)
  {
    for (running_cpu=0; running_cpu < num_cpus; running_cpu++)
    {
      ...
      { unsigned int t0_ = AAE_T(); exec_cpu(); AAE_ACUM(running_cpu & 1, t0_); }
