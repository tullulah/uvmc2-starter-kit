...
UINT32 z80intAddr;
#ifdef MZ80_PROFILE
/* The PC comes as the POINTER, not as cpu.z80pc: reading `cpu` from inside the
 * dispatch loop destabilised the emulation (the Z80 ended up pushing through a
 * wrapped SP). The base is handed over once per call, outside the loop, which is a
 * safe point — it is where mz80exec reads it anyway. */
void mz80_prof(unsigned char op, int rem, const UINT8 *pc);
void mz80_prof_base(const UINT8 *base);
#endif

/* ── SKIPPING AN IDLE SPIN ──────────────────────────────────────────────────
 *
 * A game that waits for its interrupt does so by spinning in a tight loop, and we
 * hand it the WHOLE frame's cycle budget in one slice with the IRQ at the front:
 * the moment it returns to that loop it burns everything that is left. In Tac/Scan
 * that is 94.8% of the Z80's cycles — 663 turns a frame — and every turn also goes
 * through the write handler four times.
 *
 * On a real board spinning is free. Here every spun cycle is host CPU time, so when
 * the PC reaches the loop we charge the rest of the slice and stop executing —
 * exactly what this same file already does for HALT, which is the same situation
 * stated by the program itself.
 *
 * THE CONTRACT, which is the only thing that makes this safe: the address registered
 * must be a loop THAT CAN ONLY BE LEFT BY AN INTERRUPT. If anything else can change
 * its condition, skipping the rest of the slice skips that exit too. For Tac/Scan it
 * is checked: the address it watches ($C80A) is written by exactly one PC, $007C,
 * once per frame — the IRQ handler.
 *
 * IT SKIPS ON THE SECOND ARRIVAL, NOT THE FIRST, and that is not a refinement — it
 * is what makes it work at all. The registered address is the TOP of the loop, and
 * the condition is tested further down: cutting the first time through means the
 * game never reaches its own test, never sees the interrupt's effect and never
 * leaves the loop. Measured, that reduced Tac/Scan's Z80 to 60 instructions a frame
 * — the interrupt handler and nothing else. Letting one turn through and cutting the
 * second gives up 1 turn out of 663 and is correct.
 *
 * IT IS OFF UNLESS THE BUILD ASKS FOR IT (-DMZ80_IDLE_SKIP). Every port compiles its
 * own copy of this file, so a game that has no idle loop to skip should not pay a
 * compare per instruction in the interpreter's hottest path for a feature it does
 * not use. Tac/Scan's Makefile defines it; the other 43 are untouched. */
#ifdef MZ80_IDLE_SKIP
static UINT32     dwIdlePC = 0;
static const UINT8 *pbIdle = NULL;
static int          bIdleSeen = 0;   /* this slice has been round the loop once */

void mz80_set_idle_pc(UINT32 pc)
{
	dwIdlePC = pc;
}
#else
/* Still linkable: a game may register an address in shared code that this build does
 * not act on, and that must not be a link error. */
void mz80_set_idle_pc(UINT32 pc) { (void)pc; }
#endif
#ifdef MZ80_PROFILE
#endif
UINT32 z80pc;
...
/* RP2350 fix: the 256*256*2-byte add/adc + sub/sbc flag tables (256 KB total)
 * don't fit the cart's RAM budget — malloc returned NULL and the ccpu faulted
 * on [NULL+idx]. Compute the same flag byte arithmetically instead (formula
 * lifted verbatim from mz80init's table-build loop). idx = (A<<8)|operand|(carry<<16). */
static UINT8 __attribute__((noinline)) mz80_addadc_idx(UINT32 i)
{
	UINT8 bLow = i & 0xff, bHigh = (i >> 8) & 0xff, bCarry = (i >> 16) & 1;
	UINT8 bNewAdd = bHigh + bLow + bCarry;
	UINT8 bFlag;
	if (0 == bNewAdd) bFlag = Z80_FLAG_ZERO; else bFlag = bNewAdd & 0x80;
	if (((UINT32) bLow + (UINT32) bHigh + (UINT32) bCarry) >= 0x100) bFlag |= Z80_FLAG_CARRY;
	if (((bLow ^ bHigh ^ 0x80) & (bLow ^ (bNewAdd & 0x80))) & 0x80) bFlag |= Z80_FLAG_OVERFLOW_PARITY;
	if (((bLow & 0x0f) + (bHigh & 0x0f) + bCarry) >= 0x10) bFlag |= Z80_FLAG_HALF_CARRY;
	return bFlag;
}
static UINT8 __attribute__((noinline)) mz80_subsbc_idx(UINT32 i)
{
	UINT8 bLow = i & 0xff, bHigh = (i >> 8) & 0xff, bCarry = (i >> 16) & 1;
	UINT8 bNewSub = bHigh - bLow - bCarry;
	UINT8 bFlag = Z80_FLAG_NEGATIVE;
	if (0 == bNewSub) bFlag |= Z80_FLAG_ZERO; else bFlag |= bNewSub & 0x80;
	if (((INT32) bHigh - (INT32) bLow - (INT32) bCarry) < 0) bFlag |= Z80_FLAG_CARRY;
	if (((INT32) (bHigh & 0xf) - (INT32) (bLow & 0x0f) - (INT32) bCarry) < 0) bFlag |= Z80_FLAG_HALF_CARRY;
	if (((bLow ^ bHigh) & (bHigh ^ bNewSub) & 0x80)) bFlag |= Z80_FLAG_OVERFLOW_PARITY;
	return bFlag;
}
...
static void InvalidInstruction(UINT32 dwCount)
{
	...
	dwReturnCode = (UINT32) ( (void *) pbPC - (void *) cpu.z80Base);
	...
	switch (*pbPC++)
	{
 		...
 		case 0x06:
 		{
			sdwCyclesRemaining -= 15;
				...
				while ((psMemRead->lowAddr & 0xffffffff) != 0xffffffff)
				{
					if ((cpu.z80HL >= psMemRead->lowAddr) && (cpu.z80HL <= psMemRead->highAddr))
					{
						cpu.z80pc = (UINT32) ( (void *) pbPC - (void *) cpu.z80Base);
				...
				while ((psMemWrite->lowAddr & 0xffffffff)!= 0xffffffff)
				{
			...
			sdwCyclesRemaining -= 8;
				cpu.z80F = (cpu.z80F & ~(Z80_FLAG_CARRY | Z80_FLAG_NEGATIVE | Z80_FLAG_OVERFLOW_PARITY | 
							   Z80_FLAG_HALF_CARRY | Z80_FLAG_ZERO | Z80_FLAG_SIGN)) |
								mz80_subsbc_idx(((UINT32) 0 << 8) | cpu.z80A);
				cpu.z80A = 0 - cpu.z80A;
			break;
		}
 		case 0x45:
...
cpu.z80F |= Z80_FLAG_NEGATIVE;
		...
		dwTemp |= ((UINT32) *pbPC++ << 8);
 		...
 		case 0xa1:
 		{
			sdwCyclesRemaining -= 16;
				...
				cpu.z80F |= (mz80_subsbc_idx(((UINT32) cpu.z80A << 8) | bTemp) & (Z80_FLAG_SIGN | Z80_FLAG_NEGATIVE | Z80_FLAG_ZERO));
				if (cpu.z80BC)
				{
					cpu.z80F |= Z80_FLAG_OVERFLOW_PARITY;
				}
		...
		// http://jgmalcolm.com/z80/opcodes/opcodesB#oe0
		//  http://z80-heaven.wikidot.com/instructions-set:lddr

#define CYC_WHOLE 16 // 21
#define CYC_CORRECTION 0 // 5
 		case 0xb0:
 		{
//			sdwCyclesRemaining -= 16;


			do//while ((sdwCyclesRemaining > 0) && (cpu.z80BC))
			{
				psMemRead = cpu.z80MemRead;	/* Beginning of our handler */
				while ((psMemRead->lowAddr & 0xffffffff)!= 0xffffffff)
				{
			...
			{
				sdwCyclesRemaining += CYC_CORRECTION; // if BC == 0, this one instruction only takes 16 cycles
			}
//			sdwCyclesRemaining -= 16;
			break;
		}
...
//			sdwCyclesRemaining -= 16;
			do//while ((sdwCyclesRemaining >= 0) && (cpu.z80BC))
			{
				...
				if (cpu.z80A == bTemp)
				{
					sdwCyclesRemaining+=CYC_CORRECTION;
					break;
				}
			} while ((sdwCyclesRemaining >= 0) && (cpu.z80BC));
			cpu.z80F &= ~(Z80_FLAG_ZERO | Z80_FLAG_SIGN | Z80_FLAG_HALF_CARRY | Z80_FLAG_OVERFLOW_PARITY | Z80_FLAG_NEGATIVE);
 		...
 		{
//			sdwCyclesRemaining -= 16;
			do//while ((sdwCyclesRemaining > 0) && (cpu.z80B))
			{
				psIoRead = cpu.z80IoRead;	/* Beginning of our handler */
				while (psIoRead->lowIoAddr != 0xffff)
				{
		...
		cpu.z80IX |= ((UINT32) *pbPC++ << 8);
 		...
 		case 0x84:
 		{
			sdwCyclesRemaining -= 9;
				...
				cpu.z80F = (cpu.z80F & ~(Z80_FLAG_CARRY | Z80_FLAG_NEGATIVE | Z80_FLAG_OVERFLOW_PARITY | 
							   Z80_FLAG_HALF_CARRY | Z80_FLAG_ZERO | Z80_FLAG_SIGN)) |
								mz80_addadc_idx(((UINT32) cpu.z80A << 8) | cpu.z80XH);
				InvalidInstruction(2);
			break;
		}
 		case 0x85:
 		{
			sdwCyclesRemaining -= 9;
				...
				cpu.z80F = (cpu.z80F & ~(Z80_FLAG_CARRY | Z80_FLAG_NEGATIVE | Z80_FLAG_OVERFLOW_PARITY | 
							   Z80_FLAG_HALF_CARRY | Z80_FLAG_ZERO | Z80_FLAG_SIGN)) |
								mz80_addadc_idx(((UINT32) cpu.z80A << 8) | cpu.z80XL);
				InvalidInstruction(2);
			break;
		}
 		case 0x86:
 		{
			sdwCyclesRemaining -= 19;
				...
				cpu.z80F = (cpu.z80F & ~(Z80_FLAG_CARRY | Z80_FLAG_NEGATIVE | Z80_FLAG_OVERFLOW_PARITY | 
							   Z80_FLAG_HALF_CARRY | Z80_FLAG_ZERO | Z80_FLAG_SIGN)) |
								mz80_addadc_idx(((UINT32) cpu.z80A << 8) | bTemp);
				cpu.z80A += bTemp;
			break;
		}
 		case 0x87:
 		...
 		case 0x8c:
 		{
			sdwCyclesRemaining -= 9;
				...
				cpu.z80F = (cpu.z80F & ~(Z80_FLAG_CARRY | Z80_FLAG_NEGATIVE | Z80_FLAG_OVERFLOW_PARITY | 
							   Z80_FLAG_HALF_CARRY | Z80_FLAG_ZERO | Z80_FLAG_SIGN)) |
								mz80_addadc_idx(((UINT32) cpu.z80A << 8) | cpu.z80XH | (((UINT32) cpu.z80F & Z80_FLAG_CARRY) << 16));
				InvalidInstruction(2);
			break;
		}
 		case 0x8d:
 		{
			sdwCyclesRemaining -= 9;
				...
				cpu.z80F = (cpu.z80F & ~(Z80_FLAG_CARRY | Z80_FLAG_NEGATIVE | Z80_FLAG_OVERFLOW_PARITY | 
							   Z80_FLAG_HALF_CARRY | Z80_FLAG_ZERO | Z80_FLAG_SIGN)) |
								mz80_addadc_idx(((UINT32) cpu.z80A << 8) | cpu.z80XL | (((UINT32) cpu.z80F & Z80_FLAG_CARRY) << 16));
				InvalidInstruction(2);
			break;
		}
 		case 0x8e:
 		{
			sdwCyclesRemaining -= 19;
				...
				cpu.z80F = (cpu.z80F & ~(Z80_FLAG_CARRY | Z80_FLAG_NEGATIVE | Z80_FLAG_OVERFLOW_PARITY | 
							   Z80_FLAG_HALF_CARRY | Z80_FLAG_ZERO | Z80_FLAG_SIGN)) |
								mz80_addadc_idx(((UINT32) cpu.z80A << 8) | bTemp | (((UINT32) cpu.z80F & Z80_FLAG_CARRY) << 16));
 		...
 		case 0x94:
 		{
			sdwCyclesRemaining -= 9;
				...
				cpu.z80F = (cpu.z80F & ~(Z80_FLAG_CARRY | Z80_FLAG_NEGATIVE | Z80_FLAG_OVERFLOW_PARITY | 
							   Z80_FLAG_HALF_CARRY | Z80_FLAG_ZERO | Z80_FLAG_SIGN)) |
								mz80_subsbc_idx(((UINT32) cpu.z80A << 8) | cpu.z80XH);
				InvalidInstruction(2);
			break;
		}
 		case 0x95:
 		{
			sdwCyclesRemaining -= 9;
				...
				cpu.z80F = (cpu.z80F & ~(Z80_FLAG_CARRY | Z80_FLAG_NEGATIVE | Z80_FLAG_OVERFLOW_PARITY | 
							   Z80_FLAG_HALF_CARRY | Z80_FLAG_ZERO | Z80_FLAG_SIGN)) |
								mz80_subsbc_idx(((UINT32) cpu.z80A << 8) | cpu.z80XL);
				InvalidInstruction(2);
			break;
		}
 		case 0x96:
 		{
			sdwCyclesRemaining -= 19;
				...
				cpu.z80F = (cpu.z80F & ~(Z80_FLAG_CARRY | Z80_FLAG_NEGATIVE | Z80_FLAG_OVERFLOW_PARITY | 
							   Z80_FLAG_HALF_CARRY | Z80_FLAG_ZERO | Z80_FLAG_SIGN)) |
								mz80_subsbc_idx(((UINT32) cpu.z80A << 8) | bTemp);
cpu.z80F |= Z80_FLAG_NEGATIVE;
				cpu.z80A -= bTemp;
			break;
		}
 		...
 		case 0x9c:
 		{
			sdwCyclesRemaining -= 9;
				...
				cpu.z80F = (cpu.z80F & ~(Z80_FLAG_CARRY | Z80_FLAG_NEGATIVE | Z80_FLAG_OVERFLOW_PARITY | 
							   Z80_FLAG_HALF_CARRY | Z80_FLAG_ZERO | Z80_FLAG_SIGN)) |
								mz80_subsbc_idx(((UINT32) cpu.z80A << 8) | cpu.z80XH | (((UINT32) cpu.z80F & Z80_FLAG_CARRY) << 16));
				InvalidInstruction(2);
			break;
		}
 		case 0x9d:
 		{
			sdwCyclesRemaining -= 9;
				...
				cpu.z80F = (cpu.z80F & ~(Z80_FLAG_CARRY | Z80_FLAG_NEGATIVE | Z80_FLAG_OVERFLOW_PARITY | 
							   Z80_FLAG_HALF_CARRY | Z80_FLAG_ZERO | Z80_FLAG_SIGN)) |
								mz80_subsbc_idx(((UINT32) cpu.z80A << 8) | cpu.z80XL | (((UINT32) cpu.z80F & Z80_FLAG_CARRY) << 16));
				InvalidInstruction(2);
			break;
		}
 		case 0x9e:
 		{
			sdwCyclesRemaining -= 19;
				...
				cpu.z80F = (cpu.z80F & ~(Z80_FLAG_CARRY | Z80_FLAG_NEGATIVE | Z80_FLAG_OVERFLOW_PARITY | 
							   Z80_FLAG_HALF_CARRY | Z80_FLAG_ZERO | Z80_FLAG_SIGN)) |
								mz80_subsbc_idx(((UINT32) bTemp2 << 8) | bTemp | (((UINT32) cpu.z80F & Z80_FLAG_CARRY) << 16));
cpu.z80F |= Z80_FLAG_NEGATIVE;
			break;
		}
 		case 0x9f:
...
void DDFDCBHandler(UINT32 dwWhich)
{
	...
	if (dwWhich)
	{
		dwAddr = (UINT32) ((INT32) cpu.z80IY + ((INT32) ((signed char)(*pbPC++))   )) & 0xffff;
	}
	else
	{
		dwAddr = (UINT32) ((INT32) cpu.z80IX + ((INT32) ((signed char)(*pbPC++))  )) & 0xffff;
	}
/*
	if (dwWhich)
	{
...
//	log_it(  "<ADR: %04x -> %02x>Y: Y:%04x, O:%02x", dwAddr, bTemp, (INT32) cpu.z80IY, (  (INT32) *(pbPC-1) )   );
	switch (*pbPC++)
	{
 		case 0x00:
 		{
		...
		cpu.z80IY |= ((UINT32) *pbPC++ << 8);
 		...
 		case 0x84:
 		{
			sdwCyclesRemaining -= 9;
				...
				cpu.z80F = (cpu.z80F & ~(Z80_FLAG_CARRY | Z80_FLAG_NEGATIVE | Z80_FLAG_OVERFLOW_PARITY | 
							   Z80_FLAG_HALF_CARRY | Z80_FLAG_ZERO | Z80_FLAG_SIGN)) |
								mz80_addadc_idx(((UINT32) cpu.z80A << 8) | cpu.z80YH);
				InvalidInstruction(2);
			break;
		}
 		case 0x85:
 		{
			sdwCyclesRemaining -= 9;
				...
				cpu.z80F = (cpu.z80F & ~(Z80_FLAG_CARRY | Z80_FLAG_NEGATIVE | Z80_FLAG_OVERFLOW_PARITY | 
							   Z80_FLAG_HALF_CARRY | Z80_FLAG_ZERO | Z80_FLAG_SIGN)) |
								mz80_addadc_idx(((UINT32) cpu.z80A << 8) | cpu.z80YL);
				InvalidInstruction(2);
			break;
		}
 		case 0x86:
 		...
 		case 0x8c:
 		{
			sdwCyclesRemaining -= 9;
				...
				cpu.z80F = (cpu.z80F & ~(Z80_FLAG_CARRY | Z80_FLAG_NEGATIVE | Z80_FLAG_OVERFLOW_PARITY | 
							   Z80_FLAG_HALF_CARRY | Z80_FLAG_ZERO | Z80_FLAG_SIGN)) |
								mz80_addadc_idx(((UINT32) cpu.z80A << 8) | cpu.z80YH | (((UINT32) cpu.z80F & Z80_FLAG_CARRY) << 16));
				InvalidInstruction(2);
			break;
		}
 		case 0x8d:
 		{
			sdwCyclesRemaining -= 9;
				...
				cpu.z80F = (cpu.z80F & ~(Z80_FLAG_CARRY | Z80_FLAG_NEGATIVE | Z80_FLAG_OVERFLOW_PARITY | 
							   Z80_FLAG_HALF_CARRY | Z80_FLAG_ZERO | Z80_FLAG_SIGN)) |
								mz80_addadc_idx(((UINT32) cpu.z80A << 8) | cpu.z80YL | (((UINT32) cpu.z80F & Z80_FLAG_CARRY) << 16));
				InvalidInstruction(2);
			break;
		}
 		case 0x8e:
 		...
 		case 0x94:
 		{
			sdwCyclesRemaining -= 9;
				...
				cpu.z80F = (cpu.z80F & ~(Z80_FLAG_CARRY | Z80_FLAG_NEGATIVE | Z80_FLAG_OVERFLOW_PARITY | 
							   Z80_FLAG_HALF_CARRY | Z80_FLAG_ZERO | Z80_FLAG_SIGN)) |
								mz80_subsbc_idx(((UINT32) cpu.z80A << 8) | cpu.z80YH);
				InvalidInstruction(2);
			break;
		}
 		case 0x95:
 		{
			sdwCyclesRemaining -= 9;
				...
				cpu.z80F = (cpu.z80F & ~(Z80_FLAG_CARRY | Z80_FLAG_NEGATIVE | Z80_FLAG_OVERFLOW_PARITY | 
							   Z80_FLAG_HALF_CARRY | Z80_FLAG_ZERO | Z80_FLAG_SIGN)) |
								mz80_subsbc_idx(((UINT32) cpu.z80A << 8) | cpu.z80YL);
				InvalidInstruction(2);
			break;
		}
 		case 0x96:
 		...
 		case 0x9c:
 		{
			sdwCyclesRemaining -= 9;
				...
				cpu.z80F = (cpu.z80F & ~(Z80_FLAG_CARRY | Z80_FLAG_NEGATIVE | Z80_FLAG_OVERFLOW_PARITY | 
							   Z80_FLAG_HALF_CARRY | Z80_FLAG_ZERO | Z80_FLAG_SIGN)) |
								mz80_subsbc_idx(((UINT32) cpu.z80A << 8) | cpu.z80YH | (((UINT32) cpu.z80F & Z80_FLAG_CARRY) << 16));
				InvalidInstruction(2);
			break;
		}
 		case 0x9d:
 		{
			sdwCyclesRemaining -= 9;
				...
				cpu.z80F = (cpu.z80F & ~(Z80_FLAG_CARRY | Z80_FLAG_NEGATIVE | Z80_FLAG_OVERFLOW_PARITY | 
							   Z80_FLAG_HALF_CARRY | Z80_FLAG_ZERO | Z80_FLAG_SIGN)) |
								mz80_subsbc_idx(((UINT32) cpu.z80A << 8) | cpu.z80YL | (((UINT32) cpu.z80F & Z80_FLAG_CARRY) << 16));
				InvalidInstruction(2);
			break;
		}
 		case 0x9e:
...
UINT32 mz80exec(UINT32 dwCycles)
{
	...
	/* Resolved once per slice, outside the loop: reading `cpu` from inside the
	 * dispatch destabilises the emulation (see the profiling hook's note). */
#ifdef MZ80_IDLE_SKIP
	pbIdle = dwIdlePC ? (cpu.z80Base + dwIdlePC) : NULL;
	bIdleSeen = 0;
#endif
#ifdef MZ80_PROFILE
	mz80_prof_base(cpu.z80Base);
#endif
...
again:
		bOpcode = *pbPC++;
#ifdef MZ80_IDLE_SKIP
		if (pbIdle && (pbPC - 1) == pbIdle && bIdleSeen)
		{
			/* The rest of the slice is idle. It is charged in full — as HALT does —
			 * so the caller's frame accounting does not change, and we stop
			 * executing. `pbPC` steps back so the next slice resumes here. */
			pbPC--;
			sdwCyclesRemaining = 0;
			break;
		}
		else if (pbIdle && (pbPC - 1) == pbIdle)
		{
			bIdleSeen = 1;   /* let this turn run: it is the one that tests the exit */
		}
#endif
#ifdef MZ80_PROFILE
		/* ONE HOOK, AND IT CLOSES THE PREVIOUS OPCODE. It is handed the opcode about
		 * to run and the cycle counter BEFORE it runs, so the profiler derives the
		 * previous instruction's cost from the emulator's own counter — which is the
		 * point: a `-= N` that does not match the real Z80 shows up directly as a
		 * wrong cycles/execution, without trusting anything else. Prefixed opcodes
		 * (CB/DD/ED/FD) are attributed to the prefix, which is what you want first.
		 * Inert without -DMZ80_PROFILE: this core is shared by 44 ports. */
		mz80_prof(bOpcode, sdwCyclesRemaining, pbPC - 1);
#endif
		switch (bOpcode)
		{
			case 0x00:
...
//				sdwCyclesRemaining -= 12;
			...
			case 0x80:
			{
				...
				cpu.z80F = (cpu.z80F & ~(Z80_FLAG_CARRY | Z80_FLAG_NEGATIVE | Z80_FLAG_OVERFLOW_PARITY | 
							   Z80_FLAG_HALF_CARRY | Z80_FLAG_ZERO | Z80_FLAG_SIGN)) |
								mz80_addadc_idx(((UINT32) cpu.z80A << 8) | cpu.z80B);
				cpu.z80A = bTemp2;
				break;
			}
			case 0x81:
			{
				...
				cpu.z80F = (cpu.z80F & ~(Z80_FLAG_CARRY | Z80_FLAG_NEGATIVE | Z80_FLAG_OVERFLOW_PARITY | 
							   Z80_FLAG_HALF_CARRY | Z80_FLAG_ZERO | Z80_FLAG_SIGN)) |
								mz80_addadc_idx(((UINT32) cpu.z80A << 8) | cpu.z80C);
				cpu.z80A = bTemp2;
				break;
			}
			case 0x82:
			{
				...
				cpu.z80F = (cpu.z80F & ~(Z80_FLAG_CARRY | Z80_FLAG_NEGATIVE | Z80_FLAG_OVERFLOW_PARITY | 
							   Z80_FLAG_HALF_CARRY | Z80_FLAG_ZERO | Z80_FLAG_SIGN)) |
								mz80_addadc_idx(((UINT32) cpu.z80A << 8) | cpu.z80D);
				cpu.z80A = bTemp2;
				break;
			}
			case 0x83:
			{
				...
				cpu.z80F = (cpu.z80F & ~(Z80_FLAG_CARRY | Z80_FLAG_NEGATIVE | Z80_FLAG_OVERFLOW_PARITY | 
							   Z80_FLAG_HALF_CARRY | Z80_FLAG_ZERO | Z80_FLAG_SIGN)) |
								mz80_addadc_idx(((UINT32) cpu.z80A << 8) | cpu.z80E);
				cpu.z80A = bTemp2;
				break;
			}
			case 0x84:
			{
				...
				cpu.z80F = (cpu.z80F & ~(Z80_FLAG_CARRY | Z80_FLAG_NEGATIVE | Z80_FLAG_OVERFLOW_PARITY | 
							   Z80_FLAG_HALF_CARRY | Z80_FLAG_ZERO | Z80_FLAG_SIGN)) |
								mz80_addadc_idx(((UINT32) cpu.z80A << 8) | cpu.z80H);
				cpu.z80A = bTemp2;
				break;
			}
			case 0x85:
			{
				...
				cpu.z80F = (cpu.z80F & ~(Z80_FLAG_CARRY | Z80_FLAG_NEGATIVE | Z80_FLAG_OVERFLOW_PARITY | 
							   Z80_FLAG_HALF_CARRY | Z80_FLAG_ZERO | Z80_FLAG_SIGN)) |
								mz80_addadc_idx(((UINT32) cpu.z80A << 8) | cpu.z80L);
				cpu.z80A = bTemp2;
				break;
			}
			case 0x86:
			...
			case 0x87:
			{
				...
				cpu.z80F = (cpu.z80F & ~(Z80_FLAG_CARRY | Z80_FLAG_NEGATIVE | Z80_FLAG_OVERFLOW_PARITY | 
							   Z80_FLAG_HALF_CARRY | Z80_FLAG_ZERO | Z80_FLAG_SIGN)) |
								mz80_addadc_idx(((UINT32) cpu.z80A << 8) | cpu.z80A);
				cpu.z80A = bTemp2;
				break;
			}
			case 0x88:
			{
				...
				cpu.z80F = (cpu.z80F & ~(Z80_FLAG_CARRY | Z80_FLAG_NEGATIVE | Z80_FLAG_OVERFLOW_PARITY | 
							   Z80_FLAG_HALF_CARRY | Z80_FLAG_ZERO | Z80_FLAG_SIGN)) |
								mz80_addadc_idx(((UINT32) cpu.z80A << 8) | cpu.z80B | (((UINT32) cpu.z80F & Z80_FLAG_CARRY) << 16));
				cpu.z80A = bTemp2;
				break;
			}
			case 0x89:
			{
				...
				cpu.z80F = (cpu.z80F & ~(Z80_FLAG_CARRY | Z80_FLAG_NEGATIVE | Z80_FLAG_OVERFLOW_PARITY | 
							   Z80_FLAG_HALF_CARRY | Z80_FLAG_ZERO | Z80_FLAG_SIGN)) |
								mz80_addadc_idx(((UINT32) cpu.z80A << 8) | cpu.z80C | (((UINT32) cpu.z80F & Z80_FLAG_CARRY) << 16));
				cpu.z80A = bTemp2;
				break;
			}
			case 0x8a:
			{
				...
				cpu.z80F = (cpu.z80F & ~(Z80_FLAG_CARRY | Z80_FLAG_NEGATIVE | Z80_FLAG_OVERFLOW_PARITY | 
							   Z80_FLAG_HALF_CARRY | Z80_FLAG_ZERO | Z80_FLAG_SIGN)) |
								mz80_addadc_idx(((UINT32) cpu.z80A << 8) | cpu.z80D | (((UINT32) cpu.z80F & Z80_FLAG_CARRY) << 16));
				cpu.z80A = bTemp2;
				break;
			}
			case 0x8b:
			{
				...
				cpu.z80F = (cpu.z80F & ~(Z80_FLAG_CARRY | Z80_FLAG_NEGATIVE | Z80_FLAG_OVERFLOW_PARITY | 
							   Z80_FLAG_HALF_CARRY | Z80_FLAG_ZERO | Z80_FLAG_SIGN)) |
								mz80_addadc_idx(((UINT32) cpu.z80A << 8) | cpu.z80E | (((UINT32) cpu.z80F & Z80_FLAG_CARRY) << 16));
				cpu.z80A = bTemp2;
				break;
			}
			case 0x8c:
			{
				...
				cpu.z80F = (cpu.z80F & ~(Z80_FLAG_CARRY | Z80_FLAG_NEGATIVE | Z80_FLAG_OVERFLOW_PARITY | 
							   Z80_FLAG_HALF_CARRY | Z80_FLAG_ZERO | Z80_FLAG_SIGN)) |
								mz80_addadc_idx(((UINT32) cpu.z80A << 8) | cpu.z80H | (((UINT32) cpu.z80F & Z80_FLAG_CARRY) << 16));
				cpu.z80A = bTemp2;
				break;
			}
			case 0x8d:
			{
				...
				cpu.z80F = (cpu.z80F & ~(Z80_FLAG_CARRY | Z80_FLAG_NEGATIVE | Z80_FLAG_OVERFLOW_PARITY | 
							   Z80_FLAG_HALF_CARRY | Z80_FLAG_ZERO | Z80_FLAG_SIGN)) |
								mz80_addadc_idx(((UINT32) cpu.z80A << 8) | cpu.z80L | (((UINT32) cpu.z80F & Z80_FLAG_CARRY) << 16));
				cpu.z80A = bTemp2;
				break;
			}
			case 0x8e:
			...
			case 0x8f:
			{
				...
				cpu.z80F = (cpu.z80F & ~(Z80_FLAG_CARRY | Z80_FLAG_NEGATIVE | Z80_FLAG_OVERFLOW_PARITY | 
							   Z80_FLAG_HALF_CARRY | Z80_FLAG_ZERO | Z80_FLAG_SIGN)) |
								mz80_addadc_idx(((UINT32) cpu.z80A << 8) | cpu.z80A | (((UINT32) cpu.z80F & Z80_FLAG_CARRY) << 16));
				cpu.z80A = bTemp2;
				break;
			}
			case 0x90:
			{
				...
				cpu.z80F = (cpu.z80F & ~(Z80_FLAG_CARRY | Z80_FLAG_NEGATIVE | Z80_FLAG_OVERFLOW_PARITY | 
							   Z80_FLAG_HALF_CARRY | Z80_FLAG_ZERO | Z80_FLAG_SIGN)) |
								mz80_subsbc_idx(((UINT32) cpu.z80A << 8) | cpu.z80B);
				cpu.z80A = bTemp2;
				break;
			}
			case 0x91:
			{
				...
				cpu.z80F = (cpu.z80F & ~(Z80_FLAG_CARRY | Z80_FLAG_NEGATIVE | Z80_FLAG_OVERFLOW_PARITY | 
							   Z80_FLAG_HALF_CARRY | Z80_FLAG_ZERO | Z80_FLAG_SIGN)) |
								mz80_subsbc_idx(((UINT32) cpu.z80A << 8) | cpu.z80C);
				cpu.z80A = bTemp2;
				break;
			}
			case 0x92:
			{
				...
				cpu.z80F = (cpu.z80F & ~(Z80_FLAG_CARRY | Z80_FLAG_NEGATIVE | Z80_FLAG_OVERFLOW_PARITY | 
							   Z80_FLAG_HALF_CARRY | Z80_FLAG_ZERO | Z80_FLAG_SIGN)) |
								mz80_subsbc_idx(((UINT32) cpu.z80A << 8) | cpu.z80D);
				cpu.z80A = bTemp2;
				break;
			}
			case 0x93:
			{
				...
				cpu.z80F = (cpu.z80F & ~(Z80_FLAG_CARRY | Z80_FLAG_NEGATIVE | Z80_FLAG_OVERFLOW_PARITY | 
							   Z80_FLAG_HALF_CARRY | Z80_FLAG_ZERO | Z80_FLAG_SIGN)) |
								mz80_subsbc_idx(((UINT32) cpu.z80A << 8) | cpu.z80E);
				cpu.z80A = bTemp2;
				break;
			}
			case 0x94:
			{
				...
				cpu.z80F = (cpu.z80F & ~(Z80_FLAG_CARRY | Z80_FLAG_NEGATIVE | Z80_FLAG_OVERFLOW_PARITY | 
							   Z80_FLAG_HALF_CARRY | Z80_FLAG_ZERO | Z80_FLAG_SIGN)) |
								mz80_subsbc_idx(((UINT32) cpu.z80A << 8) | cpu.z80H);
				cpu.z80A = bTemp2;
				break;
			}
			case 0x95:
			{
				...
				cpu.z80F = (cpu.z80F & ~(Z80_FLAG_CARRY | Z80_FLAG_NEGATIVE | Z80_FLAG_OVERFLOW_PARITY | 
							   Z80_FLAG_HALF_CARRY | Z80_FLAG_ZERO | Z80_FLAG_SIGN)) |
								mz80_subsbc_idx(((UINT32) cpu.z80A << 8) | cpu.z80L);
				cpu.z80A = bTemp2;
				break;
			}
			case 0x96:
			...
			case 0x97:
			{
				...
				cpu.z80F = (cpu.z80F & ~(Z80_FLAG_CARRY | Z80_FLAG_NEGATIVE | Z80_FLAG_OVERFLOW_PARITY | 
							   Z80_FLAG_HALF_CARRY | Z80_FLAG_ZERO | Z80_FLAG_SIGN)) |
								mz80_subsbc_idx(((UINT32) cpu.z80A << 8) | cpu.z80A);
				cpu.z80A = bTemp2;
				break;
			}
			case 0x98:
			{
				...
				cpu.z80F = (cpu.z80F & ~(Z80_FLAG_CARRY | Z80_FLAG_NEGATIVE | Z80_FLAG_OVERFLOW_PARITY | 
							   Z80_FLAG_HALF_CARRY | Z80_FLAG_ZERO | Z80_FLAG_SIGN)) |
								mz80_subsbc_idx(((UINT32) cpu.z80A << 8) | cpu.z80B | (((UINT32) cpu.z80F & Z80_FLAG_CARRY) << 16));
cpu.z80F |= Z80_FLAG_NEGATIVE;
				cpu.z80A = bTemp2;
				break;
			}
			case 0x99:
			{
				...
				cpu.z80F = (cpu.z80F & ~(Z80_FLAG_CARRY | Z80_FLAG_NEGATIVE | Z80_FLAG_OVERFLOW_PARITY | 
							   Z80_FLAG_HALF_CARRY | Z80_FLAG_ZERO | Z80_FLAG_SIGN)) |
								mz80_subsbc_idx(((UINT32) cpu.z80A << 8) | cpu.z80C | (((UINT32) cpu.z80F & Z80_FLAG_CARRY) << 16));
cpu.z80F |= Z80_FLAG_NEGATIVE;
				cpu.z80A = bTemp2;
				break;
			}
			case 0x9a:
			{
				...
				cpu.z80F = (cpu.z80F & ~(Z80_FLAG_CARRY | Z80_FLAG_NEGATIVE | Z80_FLAG_OVERFLOW_PARITY | 
							   Z80_FLAG_HALF_CARRY | Z80_FLAG_ZERO | Z80_FLAG_SIGN)) |
								mz80_subsbc_idx(((UINT32) cpu.z80A << 8) | cpu.z80D | (((UINT32) cpu.z80F & Z80_FLAG_CARRY) << 16));
cpu.z80F |= Z80_FLAG_NEGATIVE;
				cpu.z80A = bTemp2;
				break;
			}
			case 0x9b:
			{
				...
				cpu.z80F = (cpu.z80F & ~(Z80_FLAG_CARRY | Z80_FLAG_NEGATIVE | Z80_FLAG_OVERFLOW_PARITY | 
							   Z80_FLAG_HALF_CARRY | Z80_FLAG_ZERO | Z80_FLAG_SIGN)) |
								mz80_subsbc_idx(((UINT32) cpu.z80A << 8) | cpu.z80E | (((UINT32) cpu.z80F & Z80_FLAG_CARRY) << 16));
cpu.z80F |= Z80_FLAG_NEGATIVE;
				cpu.z80A = bTemp2;
				break;
			}
			case 0x9c:
			{
				...
				cpu.z80F = (cpu.z80F & ~(Z80_FLAG_CARRY | Z80_FLAG_NEGATIVE | Z80_FLAG_OVERFLOW_PARITY | 
							   Z80_FLAG_HALF_CARRY | Z80_FLAG_ZERO | Z80_FLAG_SIGN)) |
								mz80_subsbc_idx(((UINT32) cpu.z80A << 8) | cpu.z80H | (((UINT32) cpu.z80F & Z80_FLAG_CARRY) << 16));
cpu.z80F |= Z80_FLAG_NEGATIVE;
				cpu.z80A = bTemp2;
				break;
			}
			case 0x9d:
			{
				...
				cpu.z80F = (cpu.z80F & ~(Z80_FLAG_CARRY | Z80_FLAG_NEGATIVE | Z80_FLAG_OVERFLOW_PARITY | 
							   Z80_FLAG_HALF_CARRY | Z80_FLAG_ZERO | Z80_FLAG_SIGN)) |
								mz80_subsbc_idx(((UINT32) cpu.z80A << 8) | cpu.z80L | (((UINT32) cpu.z80F & Z80_FLAG_CARRY) << 16));
cpu.z80F |= Z80_FLAG_NEGATIVE;
				cpu.z80A = bTemp2;
				break;
			}
			case 0x9e:
			{
				...
				cpu.z80F = (cpu.z80F & ~(Z80_FLAG_CARRY | Z80_FLAG_NEGATIVE | Z80_FLAG_OVERFLOW_PARITY | 
							   Z80_FLAG_HALF_CARRY | Z80_FLAG_ZERO | Z80_FLAG_SIGN)) |
								mz80_subsbc_idx(((UINT32) cpu.z80A << 8) | bTemp | (((UINT32) cpu.z80F & Z80_FLAG_CARRY) << 16));
cpu.z80F |= Z80_FLAG_NEGATIVE;
				cpu.z80A = bTemp2;
				break;
			}
			case 0x9f:
			{
				...
				cpu.z80F = (cpu.z80F & ~(Z80_FLAG_CARRY | Z80_FLAG_NEGATIVE | Z80_FLAG_OVERFLOW_PARITY | 
							   Z80_FLAG_HALF_CARRY | Z80_FLAG_ZERO | Z80_FLAG_SIGN)) |
								mz80_subsbc_idx(((UINT32) cpu.z80A << 8) | cpu.z80A | (((UINT32) cpu.z80F & Z80_FLAG_CARRY) << 16));
				cpu.z80A = bTemp2;
cpu.z80F |= Z80_FLAG_NEGATIVE;
				break;
			}
...
cpu.z80F |= Z80_FLAG_NEGATIVE;
			...
			case 0xfb:
			{
				...
				cpu.z80iff |= IFF1;
goto again;
// MALBAN to be compatible with machine
			...
			case 0xfe:
			{
				...
				cpu.z80F = (cpu.z80F & ~(Z80_FLAG_CARRY | Z80_FLAG_NEGATIVE | Z80_FLAG_OVERFLOW_PARITY | 
							   Z80_FLAG_HALF_CARRY | Z80_FLAG_ZERO | Z80_FLAG_SIGN)) |
								mz80_subsbc_idx(((UINT32) cpu.z80A << 8) | *pbPC++);
				break;
			}
			case 0xff:
			{
...
UINT32 mz80GetRegisterValue(void *p, UINT32 reg)
{
 if (reg == CPUREG_PC)
 {
    return cpu.z80pc;
