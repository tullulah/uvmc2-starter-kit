...
/* THE DECOMPRESSION BUFFER. It used to be 1 MB "just in case", and that is 1 MB of RAM in
 * every game: moving one port to an external ROM took its footprint from 193 KB to 1312 KB,
 * and the megabyte was this, not the ROM.
 *
 * The size comes from a MEASUREMENT, not from a margin: the largest COMPRESSED member
 * across the 41 AAE romsets is 13,011 bytes. Rounded up to 32 KB, that is 2.5x of margin
 * over the known worst case and still 3% of what was there.
 *
 * Re-measure it with:
 *   python3 -c "import zipfile,glob,os,sys;d=os.path.expanduser(sys.argv[1]);
 *     print(max(i.compress_size for z in glob.glob(d+'/'+chr(42)+'.zip')
 *               for i in zipfile.ZipFile(z).infolist()))" \
 *     ~/projects/vectrex-arcade-private/arcade/roms
 *
 * And if a romset exceeds it, nothing is corrupted silently: jzReadData compares sourcelen
 * against the buffer size and returns an error. */
#define MAX_ZIP_LOAD_BUF (32*1024)
...
//        unsigned long read = zip->read(zip, comp, sourcelen);
        /* IF THE ZIP IS ALREADY IN MEMORY, puff reads from it directly and the
         * intermediate buffer is not needed. That is the cartridge case: the launcher
         * leaves the .zip in RAM, so copying it somewhere else to decompress it is paying
         * twice — and those 32 KB took three of the bigger ports out of the UVM2's 496 KB.
         * With files (host and simulator) there is no pointer to hand out, so the buffer is
         * used. */
        const unsigned char *src = zip->direct ? zip->direct(zip, sourcelen) : 0;
        unsigned long read = sourcelen;
#if defined(ROMZIP_NO_STDIO) || defined(RA_ROMZIP_NO_STDIO)
        /* Without stdio there are no zips in files: the zip is ALWAYS in memory, so the
         * buffer branch not only never runs — it must not even compile, or the linker keeps
         * zipLoadBuffer's 32 KB alive because of the reference. */
        if (!src) return Z_ERRNO;
#else
        if (!src) {
            if (sourcelen > sizeof zipLoadBuffer) return Z_ERRNO;  /* does not fit: an error, not a mess */
            read = zip->read(zip, zipLoadBuffer, sourcelen);
            src = zipLoadBuffer;
        }
#endif
        ...
        int ret = puff((unsigned char *)buffer, &destlen, src, &sourcelen);
//        free(comp);
...
/* ── FROM HERE ON, THE FILE BACKEND ─────────────────────────────────────────
 * There is no stdio on the cartridge: the zip is read from memory (the launcher leaves it
 * in RAM). Without this guard, building for bare metal gave `unknown type name 'FILE'` and
 * implicit calls to fread/ferror — which slipped through elsewhere only because those
 * builds silence the warnings. Depending on a flag in order to compile is a trap: here it
 * is said explicitly. */
#if !defined(ROMZIP_NO_STDIO) && !defined(RA_ROMZIP_NO_STDIO)
...
//    StdioJZFile *handle = (StdioJZFile *)malloc(sizeof(StdioJZFile));
    ...
    handle->handle.direct = 0;      /* reading from a file there is no pointer to hand out */
    handle->fp = fp;
}
#endif /* the file backend */
...
// return 0 on Ok
/* loadFromZip opens the zip by PATH, so it belongs to the file side too. */
#if !defined(ROMZIP_NO_STDIO) && !defined(RA_ROMZIP_NO_STDIO)
