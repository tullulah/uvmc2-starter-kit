...
/* QUANTUM'S VECTOR RAM, which every other driver reserves and never uses. It is written
 * and read by vec_ram_r8/w8/r16/w16 in quantum.c and by the memrdwdf macro above, and that
 * one is only reached under TYPE_QU. The other 43 ports pay 8 KB for an array nobody
 * reaches, so a RAM-tight target lowers it with -DAAE_VEC_RAM=4 in its Makefile. Quantum
 * does NOT touch it: left undefined, it stays at 0x1fff. */
#ifndef AAE_VEC_RAM
#define AAE_VEC_RAM 0x1fff
#endif
extern unsigned char vec_ram[AAE_VEC_RAM];
...
/* THE PALETTE, WHICH IS 12 KB OF RAM FOR 17 ENTRIES.
 *
 * `colors` is three `int`s, so 1024 entries take 12288 bytes. Whoever FILLS it never goes
 * past 16 (set_bw_colors reaches 16, the colour table 15) and whoever READS it is aae_avg.c,
 * whose largest index is the `color = 0xf + (...)` of Major Havoc's sparkle: 0x2e. The Star
 * Wars hardware games do not even look at it — swavg.c passes r,g,b straight to
 * add_color_line2i.
 *
 * It stays at 1024 by default, which is what was there: this is for a RAM-tight target, not
 * a number to be redone across the 44 ports. A game that does not fit puts
 * -DAAE_VEC_COLORS=64 in ITS Makefile. Below 64 you have to look at aae_avg.c's sparkle
 * first. */
#ifndef AAE_VEC_COLORS
#define AAE_VEC_COLORS 1024
#endif
extern colors vec_colors[AAE_VEC_COLORS];
