...
/* ── THE BEAM-TIME BUDGET, IN TICKS INSTEAD OF SECONDS ──────────────────────
 *
 * `total_time` used to be a `double` counting seconds: it started at 1/IRQ_CLOCK
 * and was decremented by 10/U51_CLOCK, 4/U51_CLOCK and — in the innermost loop, once
 * per step along every vector — 1/VCL_CLOCK.
 *
 * MEASURED on the Vectrex Studio cartridge (Tac/Scan, 430 frames): that walk was
 * 18.8 ms of a 29.2 ms frame, 64% of everything. The Cortex-M33's FPU is SINGLE
 * precision, so `double` is soft-float there no matter how the build is configured,
 * and the innermost subtraction became a library call executed tens of thousands of
 * times a frame.
 *
 * None of it needed floating point: every one of those durations is an exact integer
 * multiple of one U34 clock tick, because all three clocks are divisions of U34. In
 * ticks the same budget is exact integer arithmetic — and MORE accurate than the
 * original, which accumulated rounding over ~60000 subtractions.
 *
 * Shared by every Sega G80 port (spacfury, zektor, elim2, startrek, tacscan); the
 * change is exact, so it is safe for all of them. */
#define U34_PER_IRQ			0x1f788			/* 1/IRQ_CLOCK, in U34 ticks */
#define U34_PER_U51			32			/* 1/U51_CLOCK  (U51 = U34/16/2) */
#define U34_PER_VCL			2			/* 1/VCL_CLOCK  (VCL = U34/2)    */
...
{
	UINT8 *sintable = GI[1];
	long total_time = U34_PER_IRQ;          /* see U34_PER_* above */
	UINT16 symaddr = 0;
...
{
#ifdef MZ80_PROFILE
  /* WHO WRITES THE ADDRESS THE GAME SPINS ON. The idle loop at $7A84 only exits when
   * ($C80A) changes, and skipping the rest of the slice is only safe if the ONLY
   * writer is the once-per-frame interrupt handler. This is the place to ask: every
   * Z80 write in the machine comes through here. Inert without -DMZ80_PROFILE. */
  { void mz80_prof_watch(unsigned addr, unsigned data);
    mz80_prof_watch(address, data); }
#endif
  int op =0;
  int page=0;
  int val=0; //unsigned
